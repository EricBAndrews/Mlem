...
struct MarkdownView: View {
    ...
    var body: some View {
        Text(markdownStringFromText(text: text))
            .frame(maxWidth: .infinity, alignment: .topLeading)
        // UITextView(markdownStringFromText(text: text))
        // Text(markdownosaur.attributedString(from: Document(parsing: text)))
    }
    
    func markdownStringFromText(text: String) -> AttributedString {
        var markdownosaur = Markdownosaur()
        let document = Document(parsing: text)
        return AttributedString(markdownosaur.attributedString(from: document))
    }
}
